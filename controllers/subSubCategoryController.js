...
exports.createSubSubCategory = async (req, res) => {
  try {
    ...
    // ✅ Upload images to Cloudinary using helper
    let imagesArr = [];
    try {
      imagesArr = await uploadImages(subSubCategoryImages);
    } catch (uploadError) {
      cleanupTemporaryFiles(subSubCategoryImages);
      return sendResponse(res, "Error uploading image(s) to Cloudinary", 500, false);
    }
...
exports.updateSubSubCategory = async (req, res) => {
  try {
    ...
    // ✅ Find existing sub-subcategory
    const subSubCategory = await SubSubCategory.findById(id);
    if (!subSubCategory) {
      if (req.files?.length > 0) cleanupTemporaryFiles(req.files);
    ...
    if (name) {
      ...
      const slugExists = await SubSubCategory.findOne({
        _id: { $ne: id },
        parentSubCategory: parentSubCategoryId || subSubCategory.parentSubCategory,
        slug,
      });

      if (slugExists) {
        if (req.files?.length > 0) cleanupTemporaryFiles(req.files);
        return sendResponse(res, "Sub-subcategory with this name already exists under the same parent", 400, false);
      }
    ...
    if (subSubCategoryImages && subSubCategoryImages.length > 0) {
      try {
        ...
        if (subSubCategory.images && subSubCategory.images.length > 0) {
          await deleteOldImages(subSubCategory.images);
        }

        // Upload new images
        const newImages = await uploadImages(subSubCategoryImages);
        subSubCategory.images = newImages;
      } catch (uploadError) {
        cleanupTemporaryFiles(subSubCategoryImages);
        return sendResponse(res, "Error uploading new images to Cloudinary", 500, false);
      }
    }
...
exports.deleteSubSubCategory = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return sendResponse(res, "Invalid sub-subcategory ID", 400, false);
    }

    // ✅ Fetch sub-subcategory (lean for performance since no doc methods needed)
    const subSubCategory = await SubSubCategory.findById(id).lean();
    if (!subSubCategory) {
      return sendResponse(res, "Sub-subcategory not found", 404, false);
    }

    // ✅ Delete Cloudinary images (parallel for performance)
    if (subSubCategory.images?.length) {
      try {
        const deletePromises = subSubCategory.images.map((url) => {
          const publicId = extractPublicIdFromUrl(url);
          return publicId ? cloudinary.uploader.destroy(publicId) : null;
        });
        await Promise.all(deletePromises);
      } catch (err) {
        console.error("Error deleting sub-subcategory images from Cloudinary:", err);
        // Continue with DB delete even if Cloudinary fails
      }
    }

    // ✅ Delete sub-subcategory from DB
...
// get sub sub category by subcategory
...
// get sub sub category with related data 
...
//get sub sub category by category name 
